...
public class OrderEventPublisherV2 {
	public static void main(String[] args) throws Exception {
		...
		String key3 = "key3-" + timestamp;
		OrderCancellationEvent orderCancellation = OrderCancellationEvent.newBuilder()
								.setOrderId("order-" + timestamp)
								.setReason("no need")
								.build();
		...
		ProducerRecord<String, Object> orderCancellationRecord = new ProducerRecord<>(ORDER_EVENT_TOPIC, key3, orderCancellation);
		try {
			producer.send(placeOrderRecord);
			producer.send(orderDeliveryRecord);
			producer.send(orderCancellationRecord);
