package xiaoyf.demo.avrokafka.partyv2.consumer;

import io.confluent.kafka.serializers.AbstractKafkaSchemaSerDeConfig;
import io.confluent.kafka.serializers.KafkaAvroDeserializerConfig;
import org.apache.kafka.clients.consumer.*;
import demo.model.OrderCancellationEvent;
...
public class OrderEventListenerV2 {
	public static void main(String[] args) {
		...
		props.put(ConsumerConfig.GROUP_ID_CONFIG, OrderEventListenerV2.class.getName());
		props.put(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, SCHEMA_REGISTRY_URL_MON);
		...
		try {
			while (true) {
				...
				for (ConsumerRecord<String, Object> record : records) {
					...
					if (orderEvent instanceof OrderCancellationEvent) {
						OrderCancellationEvent orderCancellationEvent = (OrderCancellationEvent) orderEvent;
						System.out.printf(" - reason: %s\n", orderCancellationEvent.getReason());
					}
				}
				consumer.commitSync();
			}
